...
// Load JSON data
console.log('Loading bigboi_munged.json...');
const data = JSON.parse(fs.readFileSync('bigboi_munged.json', 'utf8'));
...
for (let i = 0; i < data.length; i++) {
    ...
    if (type === 'dr_step') {
        ...
        const c_idx = slam.ingest_visual_measurement(chunk);
        if (c_idx === -1) {
        ...
        if (measurement_count % 5 === 0) {
            const num_chunks = slam.get_current_chunk_count();
            const scores = slam.download_scores();
            
            const current_states = slam.download_current_particle_states();
...
// Save map to file
console.log('\nSaving map to node_baked_map.bin...');
try {
    save_map_to_file(final_map, 'node_baked_map.bin');
...
const best_trajectory = slam.download_chunk_states_for_particle(best_particle_idx, num_chunks);
...
const final_particles = slam.download_current_particle_states();
const final_measurement = slam.calculate_measurement();
...
visualizer.render(final_particles, final_measurement.mean, final_measurement.is_gaussian, {
    ...
    info: [
        ...
        `Best Score: ${Math.max(...scores).toFixed(4)}`,
        '',
        'Map saved to: node_baked_map.bin'
    ]
});

// Wait a bit before cleanup to show final state
